export const translations = {
  en: {
    hero: {
      location: 'Almaty, Kazakhstan',
      title: 'Team Lead & Senior Frontend Engineer',
      tagline: 'Building & scaling FinTech platforms',
    },
    sections: {
      about: 'About',
      experience: 'Experience',
      education: 'Education',
      projects: 'Projects',
      services: 'Services',
    },
    about: {
      bio1: `Frontend Team Lead with 10+ years building and scaling web platforms, the
      last several years focused on FinTech. I lead a frontend team at DNA
      Payments, owning architecture decisions, code review standards, and
      technical hiring, while staying hands-on with React, TypeScript, MobX,
      Next.js, and Ant Design.`,
      bio2: `Beyond delivery, I care about how teams work: mentoring engineers,
      setting up sustainable code review practices, and making architecture
      choices that hold up as products scale. Outside of my day job, I build
      personal projects with Next.js and Supabase.`,
    },
    experience: {
      'dna-lead': {
        role: 'Team Lead',
        location: 'Almaty, Kazakhstan',
        points: [
          'Lead a team of frontend engineers, owning technical direction, architecture decisions, and code quality standards for the partner platform.',
          'Conduct technical interviews and drive hiring decisions for frontend roles.',
          'Mentor engineers through code review, pairing, and architecture discussions.',
          'Own the transition of the partner dashboard to a scalable, maintainable architecture.',
        ],
      },
      'dna-senior': {
        role: 'Senior Frontend Developer',
        location: 'Almaty, Kazakhstan',
        points: [
          'Developed a partner dashboard, collaborating cross-functionally to gather requirements and define project scope.',
          'Implemented a highly responsive, user-centric frontend, driving a 20% increase in partner engagement.',
          'Used RESTful APIs to fetch and display real-time data across the dashboard.',
        ],
      },
      investax: {
        role: 'Frontend Developer',
        location: 'Singapore',
        points: [
          'Built a digital securities and security token trading platform for global private-market deals — VC, private equity, real estate, digital SPACs.',
          'Used RESTful APIs for real-time dashboard data.',
        ],
      },
      titanium: {
        role: 'Full Stack Developer',
        location: 'Almaty, Kazakhstan',
        points: [
          'Worked on projects spanning Blockchain, IoT, and ML/AI.',
        ],
      },
      halyk: {
        role: 'Senior Software Developer',
        location: 'Kazakhstan',
        points: [
          'Led development of an internal BPM platform and its frontend architecture.',
          'Built a custom Camunda tasklist and user portal, full stack.',
        ],
      },
      primesource: {
        role: 'Frontend Developer',
        location: 'Almaty, Kazakhstan',
        points: [
          'Built a seating-arrangement component for kino.kz, letting users reserve and purchase theater tickets.',
        ],
      },
      qazkom: {
        role: 'SQL Developer',
        location: 'Kazakhstan',
        points: [
          'Analyzed and developed business processes with Oracle PL/SQL, SOAP/JSON services, and a WorkFlow Builder.',
        ],
      },
    },
    education: {
      degree: "Bachelor's in Information Technology",
    },
    projects: {
      kokpar3d: {
        description: 'Browser-based 3D take on kokpar, the traditional Kazakh horseback game — real-time rendering with Three.js and a React UI layer.',
      },
      daukey: {
        description: 'Web platform for a kokpar sports club — team info, schedules, and news for the community.',
      },
      shezhire: {
        description: 'Genealogy tracker for horses — records pedigree and lineage for breeders.',
      },
      unilab: {
        description: 'Web presence for a medical organization — services, staff, and contact information.',
      },
      codequest: {
        description: 'Interactive PWA for learning programming languages through hands-on challenges.',
      },
    },
    services: {
      eyebrow: 'Services',
      heading: 'TY Group — IT solutions, done right',
      intro: "Outside my day job I run TY Group, an independent IT practice. If you need extra engineering firepower, here's how we can help:",
      items: [
        { title: 'Custom development', desc: 'Web apps, dashboards, and MVPs built with React, Next.js, TypeScript, and Node.js — from idea to shipped product.' },
        { title: 'Technical consulting', desc: "Architecture reviews, scaling advice, and hands-on help untangling a codebase that's outgrown its foundations." },
        { title: 'Code & architecture audits', desc: 'A structured look at your codebase — risks, technical debt, and a prioritized roadmap to fix it.' },
        { title: 'Team building & hiring', desc: 'Technical interviews, hiring processes, and building a frontend team that ships reliably.' },
        { title: 'Mentoring & code review', desc: "Raising a team's engineering bar through pairing, review practices, and architecture discussions." },
      ],
      forWhoLabel: 'Who we work with',
      forWho: 'Startups without an in-house tech team, businesses that need an extra expert hand, and agencies looking for outsource or outstaff frontend capacity.',
      engagementLabel: 'How we work',
      engagement: "One-off projects, ongoing consulting, or hourly engagement — whatever fits the task.",
      cta: 'Get in touch',
    },
    footer: {
      rights: '© {year} Talgat Yessenbekov',
    },
  },

  ru: {
    hero: {
      location: 'Алматы, Казахстан',
      title: 'Тимлид и старший frontend-инженер',
      tagline: 'Строю и масштабирую FinTech-платформы',
    },
    sections: {
      about: 'Обо мне',
      experience: 'Опыт',
      education: 'Образование',
      projects: 'Проекты',
      services: 'Услуги',
    },
    about: {
      bio1: `Frontend Team Lead с опытом 10+ лет в разработке и масштабировании
      веб-платформ, последние несколько лет — в FinTech. Руковожу
      frontend-командой в DNA Payments: отвечаю за архитектурные решения,
      стандарты код-ревью и технический найм, при этом остаюсь на связи с
      кодом — React, TypeScript, MobX, Next.js, Ant Design.`,
      bio2: `Помимо основной работы важно, как устроена команда: менторю
      инженеров, выстраиваю устойчивые практики код-ревью и принимаю
      архитектурные решения, которые не разваливаются при росте продукта.
      Вне работы делаю личные проекты на Next.js и Supabase.`,
    },
    experience: {
      'dna-lead': {
        role: 'Тимлид',
        location: 'Алматы, Казахстан',
        points: [
          'Руковожу командой frontend-инженеров: отвечаю за техническое направление, архитектурные решения и стандарты качества кода партнёрской платформы.',
          'Провожу технические собеседования и принимаю решения по найму frontend-специалистов.',
          'Менторю инженеров через код-ревью, парное программирование и обсуждение архитектуры.',
          'Веду переход партнёрского дашборда к масштабируемой, поддерживаемой архитектуре.',
        ],
      },
      'dna-senior': {
        role: 'Senior Frontend Developer',
        location: 'Алматы, Казахстан',
        points: [
          'Разрабатывал партнёрский дашборд, тесно взаимодействуя с кросс-функциональными командами для сбора требований и определения объёма проекта.',
          'Реализовал отзывчивый, ориентированный на пользователя интерфейс, что привело к росту вовлечённости партнёров на 20%.',
          'Использовал RESTful API для получения и отображения данных в реальном времени на дашборде.',
        ],
      },
      investax: {
        role: 'Frontend Developer',
        location: 'Сингапур',
        points: [
          'Разрабатывал платформу для торговли цифровыми ценными бумагами и security-токенами для глобальных сделок private markets — венчур, private equity, недвижимость, digital SPAC.',
          'Использовал RESTful API для данных дашборда в реальном времени.',
        ],
      },
      titanium: {
        role: 'Full Stack Developer',
        location: 'Алматы, Казахстан',
        points: [
          'Работал над проектами на стыке блокчейна, IoT и ML/AI.',
        ],
      },
      halyk: {
        role: 'Senior Software Developer',
        location: 'Казахстан',
        points: [
          'Руководил разработкой внутренней BPM-платформы и её frontend-архитектуры.',
          'Создал кастомный Camunda tasklist и пользовательский портал, full stack.',
        ],
      },
      primesource: {
        role: 'Frontend Developer',
        location: 'Алматы, Казахстан',
        points: [
          'Разработал компонент рассадки для kino.kz, позволяющий пользователям бронировать и покупать билеты в кинотеатр.',
        ],
      },
      qazkom: {
        role: 'Разработчик SQL',
        location: 'Казахстан',
        points: [
          'Анализировал и разрабатывал бизнес-процессы на Oracle PL/SQL, SOAP/JSON-сервисах и WorkFlow Builder.',
        ],
      },
    },
    education: {
      degree: 'Бакалавр, Информационные технологии',
    },
    projects: {
      kokpar3d: {
        description: 'Браузерная 3D-версия кокпара, традиционной казахской конной игры — рендеринг в реальном времени на Three.js и React-интерфейс.',
      },
      daukey: {
        description: 'Веб-платформа кокпар-клуба — информация о команде, расписание и новости для сообщества.',
      },
      shezhire: {
        description: 'Трекер родословной лошадей — фиксирует происхождение и родословную для коневодов.',
      },
      unilab: {
        description: 'Веб-присутствие медицинской организации — услуги, персонал и контактная информация.',
      },
      codequest: {
        description: 'Интерактивное PWA для изучения языков программирования через практические задания.',
      },
    },
    services: {
      eyebrow: 'Услуги',
      heading: 'TY Group — качественные IT-решения',
      intro: 'Помимо основной работы я развиваю TY Group — самостоятельную IT-практику (ИП). Если нужны дополнительные инженерные руки, вот чем можем помочь:',
      items: [
        { title: 'Разработка на заказ', desc: 'Веб-приложения, дашборды и MVP на React, Next.js, TypeScript и Node.js — от идеи до готового продукта.' },
        { title: 'Техническое консультирование', desc: 'Архитектурные ревью, советы по масштабированию и практическая помощь с кодовой базой, которая переросла свои основания.' },
        { title: 'Аудит кода и архитектуры', desc: 'Структурированный разбор кодовой базы — риски, технический долг и приоритизированный план исправлений.' },
        { title: 'Построение команды и найм', desc: 'Технические собеседования, процессы найма и формирование frontend-команды, которая стабильно выпускает продукт.' },
        { title: 'Менторство и код-ревью', desc: 'Поднимаю инженерную планку команды через парную работу, практики ревью и обсуждение архитектуры.' },
      ],
      forWhoLabel: 'С кем работаем',
      forWho: 'Со стартапами без своей технической команды, с бизнесом, которому нужна дополнительная экспертная рука, и с агентствами, которым нужен аутсорс или аутстафф frontend-разработки.',
      engagementLabel: 'Формат работы',
      engagement: 'Разовые проекты, постоянное консультирование или почасовая занятость — под задачу.',
      cta: 'Связаться',
    },
    footer: {
      rights: '© {year} Талгат Есенбеков',
    },
  },

  kz: {
    hero: {
      location: 'Алматы, Қазақстан',
      title: 'Команда жетекшісі және аға frontend-инженер',
      tagline: 'FinTech платформаларын құру және масштабтау',
    },
    sections: {
      about: 'Мен туралы',
      experience: 'Тәжірибе',
      education: 'Білім',
      projects: 'Жобалар',
      services: 'Қызметтер',
    },
    about: {
      bio1: `10+ жылдық тәжірибесі бар Frontend Team Lead-пін, веб-платформаларды
      құру және масштабтаумен айналысамын, соңғы бірнеше жыл — FinTech
      саласында. DNA Payments компаниясында frontend командасын басқарамын:
      архитектуралық шешімдерге, код-ревью стандарттарына және техникалық
      жалдауға жауаптымын, сонымен қатар React, TypeScript, MobX, Next.js,
      Ant Design технологияларымен тікелей жұмыс істеймін.`,
      bio2: `Негізгі жұмыстан бөлек, команданың қалай жұмыс істейтіні маңызды:
      инженерлерге тәлімгерлік етемін, тұрақты код-ревью тәжірибелерін
      орнатамын және өнім өскен сайын беріктігін сақтайтын архитектуралық
      шешімдер қабылдаймын. Жұмыстан тыс уақытта Next.js және Supabase
      негізінде жеке жобалар жасаймын.`,
    },
    experience: {
      'dna-lead': {
        role: 'Команда жетекшісі',
        location: 'Алматы, Қазақстан',
        points: [
          'Frontend инженерлер командасын басқарамын: серіктестік платформаның техникалық бағыты, архитектуралық шешімдері және код сапасының стандарттары үшін жауаптымын.',
          'Frontend лауазымдарына техникалық сұхбат өткіземін және жалдау шешімдерін қабылдаймын.',
          'Код-ревью, жұптық жұмыс және архитектураны талқылау арқылы инженерлерге тәлімгерлік етемін.',
          'Серіктестік дашбордын масштабталатын, қолдауға оңай архитектураға көшіремін.',
        ],
      },
      'dna-senior': {
        role: 'Senior Frontend Developer',
        location: 'Алматы, Қазақстан',
        points: [
          'Талаптарды жинау және жоба ауқымын анықтау үшін кросс-функционалды командалармен бірлесіп серіктестік дашбордын дамыттым.',
          'Пайдаланушыға бағытталған, жылдам жауап беретін интерфейс жасап, серіктестердің қатысуын 20%-ға арттырдым.',
          'Дашбордта нақты уақыттағы деректерді алу және көрсету үшін RESTful API қолдандым.',
        ],
      },
      investax: {
        role: 'Frontend Developer',
        location: 'Сингапур',
        points: [
          'Венчурлық капитал, private equity, жылжымайтын мүлік, digital SPAC сияқты жаһандық private market мәмілелеріне арналған цифрлық бағалы қағаздар мен security-токендерді сату платформасын дамыттым.',
          'Дашборд деректерін нақты уақытта көрсету үшін RESTful API қолдандым.',
        ],
      },
      titanium: {
        role: 'Full Stack Developer',
        location: 'Алматы, Қазақстан',
        points: [
          'Blockchain, IoT және ML/AI салаларын қамтитын жобалармен жұмыс істедім.',
        ],
      },
      halyk: {
        role: 'Senior Software Developer',
        location: 'Қазақстан',
        points: [
          'Ішкі BPM платформасы мен оның frontend архитектурасын дамытуды басқардым.',
          'Camunda tasklist пен пайдаланушы порталының жеке нұсқасын толық стекте жасадым.',
        ],
      },
      primesource: {
        role: 'Frontend Developer',
        location: 'Алматы, Қазақстан',
        points: [
          'kino.kz үшін орын орналасуын көрсететін компонент жасадым, ол пайдаланушыларға кинотеатр билеттерін брондауға және сатып алуға мүмкіндік береді.',
        ],
      },
      qazkom: {
        role: 'SQL Developer',
        location: 'Қазақстан',
        points: [
          'Oracle PL/SQL, SOAP/JSON қызметтері және WorkFlow Builder арқылы бизнес-процестерді талдап, дамыттым.',
        ],
      },
    },
    education: {
      degree: 'Бакалавр, Ақпараттық технологиялар',
    },
    projects: {
      kokpar3d: {
        description: 'Дәстүрлі қазақ ат үстіндегі ойыны кокпардың браузерлік 3D нұсқасы — Three.js негізінде нақты уақытта рендеринг және React интерфейсі.',
      },
      daukey: {
        description: 'Кокпар клубының веб-платформасы — команда туралы ақпарат, кесте және қауымдастыққа арналған жаңалықтар.',
      },
      shezhire: {
        description: 'Жылқылардың шежіресін тіркейтін қосымша — өсіруші үшін тектілік пен шежіре жазбалары.',
      },
      unilab: {
        description: 'Медициналық ұйымның веб-беті — қызметтер, қызметкерлер және байланыс ақпараты.',
      },
      codequest: {
        description: 'Практикалық тапсырмалар арқылы бағдарламалау тілдерін үйренуге арналған интерактивті PWA.',
      },
    },
    services: {
      eyebrow: 'Қызметтер',
      heading: 'TY Group — сапалы IT-шешімдер',
      intro: 'Негізгі жұмыстан бөлек мен TY Group атты дербес IT-практикамды жүргіземін (жеке кәсіпкер). Қосымша инженерлік күш қажет болса, біз мынада көмектесе аламыз:',
      items: [
        { title: 'Тапсырыс бойынша әзірлеу', desc: 'React, Next.js, TypeScript және Node.js негізінде веб-қосымшалар, дашбордтар және MVP — идеядан дайын өнімге дейін.' },
        { title: 'Техникалық консультация', desc: 'Архитектураны қарау, масштабтау бойынша кеңестер және өз негізінен асып кеткен кодтық базамен практикалық көмек.' },
        { title: 'Код және архитектура аудиті', desc: 'Кодтық базаны құрылымды түрде талдау — тәуекелдер, техникалық қарыз және түзету жоспарының басымдылығы.' },
        { title: 'Команда құру және жалдау', desc: 'Техникалық сұхбаттар, жалдау процестері және тұрақты нәтиже беретін frontend командасын құру.' },
        { title: 'Тәлімгерлік және код-ревью', desc: 'Жұптық жұмыс, ревью тәжірибелері және архитектураны талқылау арқылы команданың инженерлік деңгейін көтеру.' },
      ],
      forWhoLabel: 'Кіммен жұмыс істейміз',
      forWho: 'Өз техникалық командасы жоқ стартаптармен, қосымша сарапшы қолды қажет ететін бизнеспен және аутсорс немесе аутстафф frontend қуатын іздейтін агенттіктермен.',
      engagementLabel: 'Жұмыс форматы',
      engagement: 'Бір реттік жобалар, тұрақты консультация немесе сағаттық жұмыс — тапсырмаға байланысты.',
      cta: 'Хабарласу',
    },
    footer: {
      rights: '© {year} Талғат Есенбеков',
    },
  },
}

export const LANGUAGES = ['en', 'ru', 'kz']
